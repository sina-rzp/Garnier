<!-- Header -->
<?php
include('_includes/header.php');
?>



<!-- Begin Content -->
	<div id="content">
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<div class="breadcrumb"> <a href="index.php">Home</a> > <a href="fym-garnier-skin-natural-hair.php">Hair Color</a> > Olia
					</div>
				</div>
			</div>
			<div class="row">
				<div class="col-md-12">
					<div class="tajuk-besar">
						<h1>Olia</h1>
					</div>
				</div>
			</div>

<!-- Product Filter-->
                <div class="product-bar">
                  <div class="row leb-product"> 
                         <div class="col-md-6 tajuk-filter"> 
                                    Arrange according to <br/>
                        </div>
                        <div class="col-md-2">
                            <select  class="filter1 filter-btn">
                                <option value="0">Type</option>
                                <option value="per" /> Permanent<br />
                                <option value="amm" /> Ammonia-Free<br />
                            </select>
                        </div>
                        <div class="col-md-2">
                           <select  class="filter2 filter-btn">
                                <option value="0">Color </option>
                                <option value="bk" /> Black<br />
                                <option value="bro" /> Brown<br />
                                <option value="blo" /> Blonde<br />
                                <option value="red" /> Red<br />
                            </select>
                        </div>
                                   
                    </div>
                </div>




<!-- Products Listed -->

			<div class="row">
					<div class="col-xs-4 tolak-product showimg per amm bk 1">
						<div class=" a">
            				<img src="images/haircolor/Olia1-HitamAlami.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 1.0 Darkest Black</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-olia-Darkest Black.php"> &#9656; Find Out </a>
       					</div>
					</div>


					<div class="col-xs-4 tolak-product showimg  per amm bk 2">
						<div class=" a">
            				<img src="images/haircolor/Olia3.0-CoklatGelap.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  3.0 - Soft Black</b><p>The 1st oil powered Ammonia-free hair color.
							</p><a class="bar-findout" href="HC-olia-SoftBlack.php"> &#9656; Find Out </a>
       					</div>
					</div>
					<div class="col-xs-4 tolak-product showimg per amm bro 1">
						<div class=" a">
            				<img src="images/haircolor/Olia4.0-DarkBrown.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  4.0 - Dark Brown</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-olia-DarkBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>


					<div class="col-xs-4 tolak-product showimg per amm bro 1">
						<div class=" a">
            				<img src="images/haircolor/Olia4.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  4.15 - Iced Chocolate</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-IcedChocolate.php"> &#9656; Find Out </a>
       					</div>
					</div>


					<div class="col-xs-4 tolak-product showimg per amm bro 1">
						<div class=" a">
            				<img src="images/haircolor/Olia4.3GoldenDarkBrown.png" class='hold' />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  4.3- Golden Dark Brown</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-GoldenDarkBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>

					

					<div class="col-xs-4 tolak-product showimg per amm bro 1">
						<div class=" a">
            				<img src="images/haircolor/Olia5.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia  5.5 - Mahogany Brown</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-MahoganyBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>

					<div class="col-xs-4 tolak-product showimg per amm blo 1">
						<div class=" a">
            				<img src="images/haircolor/Olia5.25-FrostyChestnut.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 5.25 - Frosty Chestnut</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-olia-FrostyChestnut.php"> &#9656; Find Out </a>
       					</div>
					</div>

			

					<div class="col-xs-4 tolak-product showimg per amm bro blo 1">
						<div class=" a">
            				<img src="images/haircolor/Olia6.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 6.0 - Light Brown</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-olia-LightBrown.php"> &#9656; Find Out </a>
       					</div>
					</div>

					<div class="col-xs-4 tolak-product showimg per amm red 1">
						<div class=" a">
            				<img src="images/haircolor/Olia66.png"  />
        				</div>
				        <div class="col-xs-6 b">
				           <b>Olia 6.60 - Intense Red</b><p>The 1st oil powered Ammonia-free hair color.

							</p><a class="bar-findout" href="HC-olia-66.php"> &#9656; Find Out </a>
       					</div>
					</div>



					

					
					

				

				

			</div>

			</div>
		</div>

<!-- End OF Begin Content -->







<?php
include('_includes/footer.php');
?>



